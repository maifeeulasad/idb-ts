...
describe('QueryBuilder API - extensive tests', () => {
  ...
  beforeAll(async () => {
    ...
    await db.User.create(
      new User('u1', 'alice@example.com', 25, 'active', 100),
    );
    await db.User.create(
      new User('u2', 'bob@example.com', 17, 'inactive', 200),
    );
    await db.User.create(
      new User('u3', 'carol@example.com', 30, 'active', 300),
    );
    await db.User.create(new User('u4', 'dave@example.com', 22, 'active', 400));
    await db.User.create(
      new User('u5', 'eve@example.com', 40, 'inactive', 500),
    );
  });
  ...
  it('negative: should return empty for impossible age', async () => {
    const users = await db.User.query().where('age').equals(999).execute();
    expect(users.length).toBe(0);
  });
  ...
  it('index negative: should throw for non-existent index', async () => {
    await expect(
      db.User.query().useIndex('nonexistent').execute(),
    ).rejects.toThrow();
  });
